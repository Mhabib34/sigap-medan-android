...
class BerandaFragment : Fragment() {
    ...
    // ── Koleksi Lencana ──────────────────────────────────────────────────────
    private fun loadLencana(view: View) {
        if (!isAdded || context == null) return

        val container  = view.findViewById<LinearLayout>(R.id.containerBadge)
        container.removeAllViews()

        val semuaLencana   = db.getBadgeUser(userId)
        val lencanaUnlocked = semuaLencana.filter { it["unlocked"] == "1" }
        val adaYangUnlocked = lencanaUnlocked.isNotEmpty()

        // Kalau ada yang terbuka → tampilkan hanya yang terbuka
        // Kalau belum ada satu pun → tampilkan semua dengan ikon gembok (placeholder)
        val tampilList = if (adaYangUnlocked) lencanaUnlocked else semuaLencana

        tampilList.forEach { lencana ->
            val isUnlocked = adaYangUnlocked // kalau masuk sini semua sudah unlocked
            // Kalau belum ada yang unlock, semua tampil sebagai gembok
            val showAsLocked = !adaYangUnlocked
            ...
            val cardView = androidx.cardview.widget.CardView(requireContext()).apply {
                ...
                setCardBackgroundColor(
                    if (showAsLocked) Color.parseColor("#E0E0E0")
                    else Color.parseColor(lencana["warna"] ?: "#F0F0F0")
                )
            }
            ...
            val tvIcon = TextView(requireContext()).apply {
                ...
                // Gembok jika placeholder, ikon asli jika sudah terbuka
                text     = if (showAsLocked) "🔒" else lencana["icon"]
                textSize = 24f
                gravity  = android.view.Gravity.CENTER
                alpha    = if (showAsLocked) 0.45f else 1f
            }
            ...
            val tvNama = TextView(requireContext()).apply {
                layoutParams = LinearLayout.LayoutParams(
                    ...
                    LinearLayout.LayoutParams.WRAP_CONTENT
                ).also { it.topMargin = 6.dpToPx() }
                text     = if (showAsLocked) "TERKUNCI" else lencana["nama"]?.uppercase()
                textSize = 9f
                setTextColor(Color.parseColor(if (showAsLocked) "#BBBBBB" else "#888888"))
                gravity  = android.view.Gravity.CENTER
            }

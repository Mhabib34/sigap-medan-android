...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("""
            CREATE TABLE $TABLE_LAPORAN (
                ...
                $COL_LAP_LAT REAL,
                $COL_LAP_LNG REAL,
                $COL_LAP_KATEGORI TEXT DEFAULT 'laporan',
        ...
        db.execSQL("""
            CREATE TABLE $TABLE_VOTES (
                ...
                $COL_VOT_REPORT_ID TEXT NOT NULL,
                $COL_VOT_USER_ID INTEGER NOT NULL,
                $COL_VOT_TANGGAL TEXT NOT NULL,
                UNIQUE($COL_VOT_REPORT_ID, $COL_VOT_USER_ID)
            )
        """.trimIndent())
    ...
    fun tambahVote(reportId: String, userId: Int): Boolean {
        ...
        val values = ContentValues().apply {
            put(COL_VOT_REPORT_ID, reportId)
            put(COL_VOT_USER_ID, userId)
            put(COL_VOT_TANGGAL, tanggal)
        }
        val result = db.insertWithOnConflict(
            TABLE_VOTES, null, values, SQLiteDatabase.CONFLICT_IGNORE
        )
        db.close()
        return result != -1L
    }
    ...
    fun sudahVote(reportId: String, userId: Int): Boolean {
        ...
        val cursor = db.query(
            TABLE_VOTES,
            arrayOf(COL_VOT_ID),
            "$COL_VOT_REPORT_ID = ? AND $COL_VOT_USER_ID = ?",
            arrayOf(reportId, userId.toString()),
            null, null, null
        )
        val sudah = cursor.count > 0
        cursor.close()
        db.close()
        return sudah
    }
    ...
    fun getJumlahVote(reportId: String): Int {
        ...
        val cursor = db.rawQuery(
            "SELECT COUNT(*) FROM $TABLE_VOTES WHERE $COL_VOT_REPORT_ID = ?",
            arrayOf(reportId)
        )
        cursor.moveToFirst()
    ...
    fun getJumlahVoteBatch(reportIds: List<String>): Map<String, Int> {
        ...
        val cursor = db.rawQuery(
            """SELECT $COL_VOT_REPORT_ID, COUNT(*) as total
               FROM $TABLE_VOTES
               WHERE $COL_VOT_REPORT_ID IN ($placeholders)
               GROUP BY $COL_VOT_REPORT_ID""",
    ...
    private fun insertDefaultBadge(db: SQLiteDatabase) {
        val badgeList = listOf(
            arrayOf("Pahlawan Kota",    "🏅", "#FFF3EE", "laporan",      "1"),
            arrayOf("Pejuang Hijau",    "🌿", "#EEFAF5", "lingkungan",   "1"),
            arrayOf("Warga Peduli",     "🤝", "#F0F0F0", "sosial",       "3"),
            arrayOf("Pengguna Setia",   "🚌", "#FFFBE6", "transportasi", "1")
        )
        badgeList.forEach {
            db.execSQL("""
    ...
    fun getUserByEmail(email: String): Map<String, String>? {
        ...
        return if (cursor.moveToFirst()) {
            ...
            cursor.close(); db.close(); data
        } else { cursor.close(); db.close(); null }
    }
    ...
    fun isMisiSelesai(userId: Int, misiId: Int): Boolean {
        ...
        val cursor = db.query(TABLE_MISI_USER, arrayOf(COL_MU_ID),
            ...
            arrayOf(userId.toString(), misiId.toString(), "$today%"), null, null, null)
    ...
    fun selesaikanMisi(userId: Int, misiId: Int): Int {
        ...
        val cursor = misiDb.query(TABLE_MISI,
            arrayOf(COL_MISI_POIN, COL_MISI_JUDUL, COL_MISI_KATEGORI),
            "$COL_MISI_ID = ?", arrayOf(misiId.toString()), null, null, null)
        var poin = 0; var judul = ""; var kategoriMisi = ""
        if (cursor.moveToFirst()) {
            ...
            kategoriMisi = cursor.getString(cursor.getColumnIndexOrThrow(COL_MISI_KATEGORI))
        }
        cursor.close()
        misiDb.close()
        if (poin > 0) {
    ...
    fun getAllLaporan(): List<Map<String, String>> {
        ...
        ORDER BY $COL_LAP_KATEGORI ASC, $COL_LAP_TANGGAL DESC
    """.trimIndent(), null)
        val list = mutableListOf<Map<String, String>>()
        while (cursor.moveToNext()) {
            list.add(mapOf(
                ...
                "kategori" to (cursor.getString(cursor.getColumnIndexOrThrow(COL_LAP_KATEGORI)) ?: "laporan"),
    ...
    fun getLaporanKemarin(): Int {
        ...
        val kemarin = java.text.SimpleDateFormat("yyyy-MM-dd",
    ...
    fun getThresholds(allReports: List<Map<String, String>>): Pair<Int, Int> {
        ...
        val maxSize = clusterSizes.maxOrNull() ?: 1
        return Pair(
            (maxSize * 0.70).toInt().coerceAtLeast(2),
            (maxSize * 0.35).toInt().coerceAtLeast(1)
        )
    }
    ...
    fun getAllMisi(): List<Map<String, String>> {
        val list = mutableListOf<Map<String, String>>()
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT * FROM misi ORDER BY id ASC", null)
        while (cursor.moveToNext()) {
